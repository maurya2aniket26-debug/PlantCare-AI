...
# ============================================================
# ADMIN SUPPORT
# Adds an "is_admin" column to the users table (safe to run
# every time the app starts - it only adds the column if it
# doesn't already exist) and creates a default admin account
...
def create_default_admin():
    ...
    try:
        ...
        if existing:
            ...
            # Make sure this account is (still) flagged as admin.
            if DATABASE_URL:
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if user and check_password_hash(user[2], password) and bool(user[3]):
...
# ============================================================
# ADMIN ACCESS GUARD (shared by every /admin/* page)
# ============================================================
...
def admin_dashboard():
    ...
    if DATABASE_URL:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM history WHERE LOWER(disease) = 'healthy'"
        )
    ...
    healthy_predictions = cursor.fetchone()[0]
...
def admin_delete_user(user_id):
    ...
    # Safety: don't allow an admin to delete their own account
    # while logged in with it.
    ...
    try:
        ...
        row = cursor.fetchone()
        deleted_username = row[0] if row else f"user #{user_id}"
    ...
    log_admin_action(
        "Deleted user",
        f"Removed user '{deleted_username}' (id {user_id}) and their prediction history."
    )
...
def admin_predictions():
    ...
    cursor.execute(
        """
        SELECT history.id, users.username, history.plant, history.disease,
...
def admin_activity():
    ...
    cursor.execute(
        """
        SELECT users.username, history.plant, history.disease, history.created_at
        FROM history
        JOIN users ON users.id = history.user_id
        ORDER BY history.created_at DESC
        LIMIT 10
...
# ============================================================
# ADMIN - SETTINGS (change own username / password)
# ============================================================
...
def admin_settings():
    ...
    if request.method == "POST":
        ...
        if not row or not check_password_hash(row[1], current_password):
            ...
            final_username = new_username if new_username else row[0]
            updates_made = []

            try:

                if new_username and new_username != row[0]:
                ...
                if not error:
                    ...
                    if updates_made:
                        ...
                        success = "Updated: " + ", ".join(updates_made) + "."

                        log_admin_action(
                            "Updated admin credentials",
                            f"Changed: {', '.join(updates_made)}"
                        )

                    else:

                        error = "Nothing to update — enter a new username and/or password."
